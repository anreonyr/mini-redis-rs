...
pub async fn dispatch_command(cmd: Result<ParsedCmd, CmdError>) -> resp::RespType {
    ...
    match parsed {
        ...
        ParsedCmd::Setrange { key, offset, value } => handlers::handle_setrange(&key, offset, &value),
        ...
        ParsedCmd::Rpoplpush { source, destination } => handlers::handle_rpoplpush(&source, &destination),
        ...
        ParsedCmd::Hincrbyfloat { key, field, incr } => handlers::handle_hincrbyfloat(&key, &field, incr),
        ...
        ParsedCmd::Zincrby { key, incr, member } => handlers::handle_zincrby(&key, incr, &member),
        ...
        ParsedCmd::Zremrangebyrank { key, start, stop } => handlers::handle_zremrangebyrank(&key, start, stop),
        ParsedCmd::Zremrangebyscore { key, min, max } => handlers::handle_zremrangebyscore(&key, &min, &max),
